<script>
  ...
  $: if(width) small = width < 900
  $: if(width) medium = width >= 900 && width < 1200
  ...
  let simulation 
  let simulationType = undefined
  $: if (nodes && linkForce && radialForce && collideForce && xForce && yForce && typeof(small) !== undefined && small) {
  ...
  $: if (typeof(width) !== undefined && simulationType === 'mobile') {
    //simulation.restart()
    simulation.tick(NUM_ITERATIONS)
    simulation.stop()
    simulationFinished = true
  }
  $: if (typeof(width) !== undefined && simulationType === 'desktop') {
    //simulation.restart()
    simulation.tick(NUM_ITERATIONS)
    simulation.stop()
    simulationFinished = true
